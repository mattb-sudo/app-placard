...
type SubCategory = (typeof SUBCATS)[keyof typeof SUBCATS][number];
...
function normalizeProductRelation(
  product: ProductRow | ProductRow[] | null | undefined,
): Product | null {
...
const LEGACY_SETTINGS_STORAGE_KEY = "pantrypilot_settings_v1";
const LEGACY_SHOPPING_CHECKED_STORAGE_KEY = "pantrypilot_checked_shopping_v1";
const SETTINGS_STORAGE_KEY = "vplacard_settings_v1";
const SHOPPING_CHECKED_STORAGE_KEY = "vplacard_checked_shopping_v1";
const MEDIA_BUCKET = "vplacard-media";
const IMAGE_FILE_ACCEPT = "image/jpeg,image/png,image/webp,image/gif";
const SUPABASE_PUBLIC_URL = String(
...
const APP_IMAGE_FIELDS: {
  ...
  { key: "historyHero", label: "Historique", description: "Anciens achats" },
  {
    ...
    description: "Aucun ancien achat",
  },
];
    ...
    .map((ingredient) => ({
      ...
      unit:
        typeof ingredient.unit === "string"
          ? toIngredientUnit(ingredient.unit)
          : null,
    }))
...
function normalizeRecipeOverrideRow(
  row: RecipeOverrideRow,
  ...
  const servings =
    row.servings != null && Number.isFinite(Number(row.servings))
      ? Number(row.servings)
      : null;
...
const SAMPLE_RECIPES: Recipe[] = [
  ...
  },
  {
    ...
    kind: "savory",
    ingredients: [
      qi("courgette", 2, "unité"),
      qi("aubergine", 1, "unité"),
      qi("tomate", 400, "g"),
      qi("poivron", 2, "unité"),
      qi("oignon", 1, "unité"),
    ],
  ...
  {
    id: "tarte-thon-tomate",
    name: "Tarte thon tomate",
    kind: "savory",
    ingredients: [
      qi("pate brisee", 1, "unité"),
      qi("thon", 160, "g"),
...
function recipeImageFor(
  ...
  const index =
    recipe.id.split("").reduce((total, char) => total + char.charCodeAt(0), 0) %
    pool.length;
...
function gramsFromStock(item: StockItem): {
  grams: number | null;
  approx: boolean;
} {
  ...
  if (unit === "l") {
    ...
    return { grams, approx: p.density_g_ml == null };
  }
...
function kcalForStock(item: StockItem): {
  kcal: number | null;
  approx: boolean;
} {
  const p = item.product;
...
function mealCaloriesInfo(
  ...
  const mealIngredients =
    meal.ingredients?.map(parseSingleMealIngredient) ?? [];
...
function parseRecipeIngredients(text: string): RecipeIngredient[] {
  return text
    ...
    .map((part) => {
      ...
      const parsedAmount = m?.[2]
        ? Number(String(m[2]).replace(",", "."))
        : null;
      const amount =
...
function scaleRecipeIngredients(
  ...
  const targetServings =
    mealServings && mealServings > 0 ? mealServings : baseServings;
...
function App() {
  ...
  const [checkedShoppingIds, setCheckedShoppingIds] = useState<string[]>(() => {
    try {
      const raw = readLocalStorageWithFallback(
        SHOPPING_CHECKED_STORAGE_KEY,
        LEGACY_SHOPPING_CHECKED_STORAGE_KEY,
      );
  ...
  const [stockSortDirection, setStockSortDirection] =
    useState<StockSortDirection>("asc");
  const [recipesSubTab, setRecipesSubTab] = useState<"feasible" | "all">(
    "feasible",
  );
  ...
  >({});
  const [imageUploadingKey, setImageUploadingKey] = useState<string | null>(
    null,
  );
  ...
  const [addExistingProductId, setAddExistingProductId] = useState<
    string | null
  >(null);
  const [showBackToShoppingAfterAdd, setShowBackToShoppingAfterAdd] =
    useState(false);
  ...
  const [newRecipeImageFile, setNewRecipeImageFile] = useState<File | null>(
    null,
  );
  ...
  const [editRecipeImageFile, setEditRecipeImageFile] = useState<File | null>(
    null,
  );
  ...
  const [editExpirationType, setEditExpirationType] =
    useState<ExpirationType>("dlc");
  ...
  const [planRecipeValue, setPlanRecipeValue] = useState<string>("");
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        SHOPPING_CHECKED_STORAGE_KEY,
        JSON.stringify(checkedShoppingIds),
      );
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    return () => {
      if (newRecipeImagePreviewRef.current)
        URL.revokeObjectURL(newRecipeImagePreviewRef.current);
      if (editRecipeImagePreviewRef.current)
        URL.revokeObjectURL(editRecipeImagePreviewRef.current);
    };
  }, []);
  ...
  const resetImageAsset = async (kind: ImageAssetKind, imageKey: string) => {
    ...
    if (kind === "app" && isAppImageKey(imageKey)) {
      setAppImages((prev) => ({
        ...prev,
        [imageKey]: DEFAULT_APP_IMAGES[imageKey],
      }));
    } else if (kind === "recipe") {
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => {
      addProductSectionRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
  ...
  const openEdit = (item: StockItem) => {
    ...
    setEditKcal100g(
      item.product?.kcal_100g != null ? String(item.product.kcal_100g) : "",
    );
    setEditGramsPerUnit(
      item.product?.grams_per_unit_g != null
        ? String(item.product.grams_per_unit_g)
        : "",
    );
    setEditDensityGml(
      item.product?.density_g_ml != null
        ? String(item.product.density_g_ml)
        : "",
    );
  ...
  const saveEdit = async () => {
    ...
    try {
            ...
            ? {
                ...
                product: s.product
                  ? {
                      ...
                      category: editCategory ? editCategory : null,
                      sub_category:
                        editCategory === "Produit sucré" ||
                        editCategory === "Produit salé"
                          ? editSubCategory || null
                          : null,
                      barcode: editBarcode.trim() || null,
                      kcal_100g: editKcal100g ? Number(editKcal100g) : null,
                      grams_per_unit_g: editGramsPerUnit
                        ? Number(editGramsPerUnit)
                        : null,
                      density_g_ml: editDensityGml
                        ? Number(editDensityGml)
                        : null,
                    }
                  : null,
              }
      ...
      setError(
        message ? `Erreur: ${message}` : "Erreur lors de l'enregistrement.",
      );
    } finally {
      setEditSaving(false);
    }
  ...
  const fetchWeekMeals = async (start: Date) => {
    ...
    const { data, error } = await supabase
      .from("week_meals")
      .select(
        "id, meal_date, meal_slot, recipe_id, recipe_name, recipe_kind, ingredients, kcal_override, servings, notes, consumed_at",
      )
      .gte("meal_date", from)
      .lte("meal_date", to)
    ...
    const normalized: WeekMeal[] = ((data ?? []) as WeekMealRow[]).map((r) => ({
      ...
      ingredients: Array.isArray(r.ingredients)
        ...
        : null,
      kcal_override: r.kcal_override ?? null,
  ...
  useEffect(() => {
    ...
    else if (subCategory && !getSubcatsFor(category).includes(subCategory))
      setSubCategory("");
  ...
  useEffect(() => {
    if (getSubcatsFor(editCategory).length === 0) setEditSubCategory("");
      ...
      editSubCategory &&
      !getSubcatsFor(editCategory).includes(editSubCategory)
    )
      setEditSubCategory("");
  ...
  const autofillFromBarcode = async (code: string) => {
    ...
    try {
      ...
      if (json.status !== 1) {
        setError(
          "Produit introuvable dans Open Food Facts. Remplis les infos à la main.",
        );
        return;
      }
  ...
  const upsertWeekMeal = async (
    payload: Omit<WeekMeal, "id" | "consumed_at">,
  ) => {
    setError(null);
    ...
    const row: WeekMeal = {
      ...
      ingredients: Array.isArray(data.ingredients)
        ...
        : null,
      kcal_override: data.kcal_override ?? null,
    ...
    setWeekMeals((prev) => {
      const idx = prev.findIndex(
        (m) => m.meal_date === row.meal_date && m.meal_slot === row.meal_slot,
      );
  ...
  const deleteWeekMeal = async (meal_date: string, meal_slot: MealSlot) => {
    ...
    setWeekMeals((prev) =>
      prev.filter(
        (m) => !(m.meal_date === meal_date && m.meal_slot === meal_slot),
      ),
    );
  };
  ...
  const markWeekMealConsumed = async (meal: WeekMeal) => {
    ...
    setWeekMeals((prev) =>
      prev.map((m) =>
        m.id === meal.id
          ? { ...m, consumed_at: data.consumed_at ?? consumedAt }
          : m,
      ),
    );
    ...
    if (stockResult.missing.length > 0) {
      try {
        const createdCount = await addMissingProductsSilently(
          stockResult.missing,
          meal.recipe_kind ?? "savory",
        );
        ...
        console.error(e);
        shoppingMessage =
          " Impossible d'ajouter les ingrédients manquants à la liste de courses.";
      }
    }
  ...
  const handleBarcodeDetected = (raw: string) => {
    ...
    if (!cleaned) {
      setError(
        "Scan illisible : aucun chiffre détecté. Réessaie en visant mieux le code-barres.",
      );
      return;
    }
    setBarcode(cleaned);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (stockError) {
        ...
        const normalized: StockItem[] = ((stockData ?? []) as StockRow[]).map(
          normalizeStockRow,
        );
  ...
  const saveRecipeOverride = async (
    recipeId: string,
  ...
  const createRecipeInDb = async () => {
    ...
    const servings =
      Number.isFinite(servingsNum) && servingsNum > 0 ? servingsNum : null;
  ...
  const openEditRecipe = (recipe: Recipe) => {
    ...
    setEditRecipeIngredients(
      recipe.ingredients.map(formatRecipeIngredient).join(", "),
    );
  ...
  const saveEditRecipe = async () => {
    ...
    const isDbRecipe = dbRecipes.some(
      (recipe) => recipe.id === editRecipeTarget.id,
    );
    ...
    if (insertIngredientsError) {
      ...
      setError(
        "Recette mise à jour, mais erreur sur les nouveaux ingrédients.",
      );
      return;
    }
  ...
  const unhideFromShopping = async (productId: string) => {
    ...
    setProducts((prev) =>
      prev.map((p) =>
        p.id === productId ? { ...p, shopping_hidden: false } : p,
      ),
    );
    ...
    setStocks((prev) =>
      prev.map((s) =>
        s.product?.id === productId
          ? { ...s, product: { ...s.product, shopping_hidden: false } }
          : s,
      ),
    );
  };
  ...
  const handleAdd = async (e: FormEvent) => {
    ...
    try {
      ...
      if (addExistingProductId) {
        const { data: existingProductById, error: existingProductByIdError } =
          await supabase
            .from("products")
            .select(
              `id,name,generic_name,brand,category,sub_category,default_unit,barcode,shopping_hidden,is_main,kcal_100g, kcal_serving, serving_size_g, grams_per_unit_g, density_g_ml`,
      ...
      if (!productRow && trimmedBarcode) {
        const { data: existingProducts, error: existingProductError } =
          await supabase
            .from("products")
            .select(
              `id,name,generic_name,brand,category,sub_category,default_unit,barcode,shopping_hidden,is_main,kcal_100g, kcal_serving, serving_size_g, grams_per_unit_g, density_g_ml`,
        ...
        if (existingProducts && existingProducts.length > 0)
          productRow = existingProducts[0] as ProductRow;
      }
      ...
      if (!productRow) {
        const { data: productData, error: productError } = await supabase
          ...
          .insert({
            ...
            sub_category:
              getSubcatsFor(category).length > 0 ? subCategory || null : null,
      ...
      const { data: existingStocks, error: existingStocksError } =
      ...
      const addedFromShoppingProductId = addExistingProductId
      ...
      if (addedFromShoppingProductId) {
        setCheckedShoppingIds((prev) =>
          prev.filter((id) => id !== addedFromShoppingProductId),
        );
      }
  ...
  const recipeKindToCategory = (kind: RecipeKind) =>
  ...
  const ensureProductExistsForShopping = async (
    ...
    if (createError || !created)
      throw createError || new Error("Erreur création produit");
  ...
  const addMissingProductsSilently = async (
    missing: string[],
    kind: RecipeKind,
  ): Promise<number> => {
    let createdCount = 0;
  ...
  const soonItems = inStock.filter(
    (i) => getExpirationStatus(i.expiration_date, settings.soonDays) === "soon",
  ).length;
  const expiredItems = inStock.filter(
    (i) =>
      getExpirationStatus(i.expiration_date, settings.soonDays) === "expired",
  ).length;

  const soonList = inStock
    .filter(
      (i) =>
        getExpirationStatus(i.expiration_date, settings.soonDays) === "soon",
    )
    .sort((a, b) =>
  ...
  async function decrementStockForMeal(
    meal: WeekMeal,
  ): Promise<{ decremented: string[]; missing: string[] }> {
    ...
    for (const ingredient of mealIngredients) {
      ...
      const candidates = stocks
        .filter(
          (s) =>
            (s.quantity ?? 0) > 0 && s.product?.name && !usedStockIds.has(s.id),
        )
        .filter((s) => {
        ...
        .sort((a, b) => {
          const score = (stock: StockItem) => {
            ...
            if (status === "expired" && stock.expiration_type === "dlc")
              return 0;
      ...
      for (const stock of candidates) {
        ...
        const next = Math.max(
          0,
          roundQty(current - amountForThisStock, stock.unit),
        );
        ...
        remainingToSubtract = roundQty(
          remainingToSubtract - amountForThisStock,
          stock.unit,
        );
      }
      ...
      if (remainingToSubtract > 0) {
        missingIngredients.push(
          `${parsedIngredient.name} ${remainingToSubtract} ${lastUnitLabel}`,
        );
      }
    }
  ...
  const todayMeals = MEAL_SLOTS.map((slot) => ({
    ...
    meal:
      weekMeals.find(
        (meal) => meal.meal_date === todayKey && meal.meal_slot === slot.key,
      ) ?? null,
  }));
  ...
  const plannedTodayMeals = todayMeals.filter(
    ({ meal }) => meal && !meal.consumed_at,
  );
  ...
  const todayCaloriesPercent =
    settings.dailyCalorieGoal > 0
      ? Math.min(
          100,
          Math.round((todayCalories / settings.dailyCalorieGoal) * 100),
        )
      : 0;
  ...
  const renderHistoryTab = () => {
    ...
    const historyPlaces = new Set(
      outOfStock.map((item) => item.place).filter(Boolean),
    );
    const historyCategories = new Set(
      outOfStock.map((item) => item.product?.category).filter(Boolean),
    );
    ...
    const groupedByCategory: { label: string; items: StockItem[] }[] =
    ...
    const uncategorizedItems = outOfStock.filter(
      (item) => !categorizedHistoryIds.has(item.id),
    );
    ...
    const renderHistoryItem = (item: StockItem) => (
      <article key={item.id} className="history-product-card">
        <div className="history-product-main">
          <span className="history-product-category">
            {item.product?.category ?? "Sans catégorie"}
          </span>
          <h3>{item.product?.name ?? "Produit"}</h3>
          <p>
            {item.product?.brand ||
              item.product?.generic_name ||
              "Ancien achat enregistré"}
          </p>
        </div>
      ...
      <>
        <div className="main-header">
          <div>
            ...
            <p className="main-subtitle">
              Les produits déjà connus à remettre au stock rapidement.
            </p>
          </div>
        ...
        <section className="history-hero">
          ...
          <div className="history-hero-content">
            ...
            <p>
              Reprends un ancien achat sans recréer sa fiche, puis ajuste
              simplement quantité, unité et lieu.
            </p>
          </div>
        ...
        {outOfStock.length === 0 ? (
          <section className="history-empty-state">
            ...
            <div>
              ...
              <p>
                Les produits qui tombent à zéro apparaîtront ici pour être remis
                au stock plus vite.
              </p>
              <button
                type="button"
  ...
  const renderWeekMenuTab = () => {
    ...
    const getCell = (dateKey: string, slot: MealSlot) =>
      weekMeals.find((m) => m.meal_date === dateKey && m.meal_slot === slot) ??
      null;
    ...
    const openPlan = (dateKey: string, slot: MealSlot) => {
      ...
      setPlanSlot(slot);
      setPlanKcalOverride(
        existing?.kcal_override != null ? String(existing.kcal_override) : "",
      );

      if (existing) {
        setPlanRecipeValue(
          existing.recipe_id ? `db:${existing.recipe_id}` : "custom",
        );
    ...
    const savePlan = async () => {
      ...
      if (!v || v === "custom") {
        ...
        ingredients = r?.ingredients
          ? scaleRecipeIngredients(r.ingredients, r.servings, servings).map(
      ...
      const kcal_override = Number.isFinite(kcalOverrideNum)
        ? kcalOverrideNum
        : null;
    ...
    const mealsThisWeek = weekMeals.filter((meal) =>
      dayKeys.includes(meal.meal_date),
    );
    const consumedWeekMeals = mealsThisWeek.filter(
    ...
    const freeWeekSlots = Math.max(
      0,
      dayKeys.length * MEAL_SLOTS.length - mealsThisWeek.length,
    );
    const completedWeekPercent =
      mealsThisWeek.length > 0
        ? Math.round((consumedWeekMeals / mealsThisWeek.length) * 100)
        : 0;
    const averageMealCalories =
      mealsThisWeek.length > 0 && estimatedWeekCalories > 0
    ...
    const nextPlannedEntry = days
      .flatMap((day) => {
        ...
        return getDayMealEntries(dateKey).map(({ slot, meal }) => ({
          day,
          dateKey,
          slot,
          meal,
      ...
      .find(
        ({ dateKey, meal }) => meal && !meal.consumed_at && dateKey >= todayKey,
      );
    ...
    return (
      <>
        ...
        <section
          ...
          style={
            {
              "--planner-progress": `${completedWeekPercent}%`,
            } as CSSProperties
          }
        >
          ...
          <div className="planner-band-content">
            ...
            <p>
              {weekRangeLabel} ·{" "}
              {pluralize(consumedWeekMeals, "repas validé", "repas validés")} ·{" "}
              {pluralize(freeWeekSlots, "créneau libre", "créneaux libres")}
            </p>
          ...
          <div className="planner-band-side">
            ...
            <button
              ...
              onClick={() => openPlan(dayKeys[0], "lunch")}
            >
              <span
                className="btn-symbol btn-symbol--calendar"
                aria-hidden="true"
              />
              Composer un repas
            </button>
          ...
          <div className="planning-toolbar">
            ...
            <div className="planning-toolbar-actions">
              ...
              <button
                ...
                onClick={() => void addWeekMissingToShopping()}
              >
          ...
          {weekMealsLoading ? (
            ...
            <div className="planning-week-grid">
              {days.map((day) => {
                ...
                const plannedDayMeals = dayEntries.filter(
                  ({ meal }) => meal,
                ).length;
                const consumedDayMeals = dayEntries.filter(
                  ({ meal }) => meal?.consumed_at,
                ).length;
                ...
                return (
                  ...
                  >
                    <div className="planning-day-head">
                      ...
                      <small>
                        {isToday ? "Aujourd'hui" : `${plannedDayMeals}/3`}
                      </small>
                    </div>
                    ...
                    <div className="planning-day-summary">
                      ...
                      <span>
                        {pluralize(consumedDayMeals, "validé", "validés")}
                      </span>
                    </div>
                    ...
                    <div className="planning-meal-stack">
                      {dayEntries.map(({ slot, meal }) => {
                        const calories = meal
                          ? mealCaloriesInfo(meal, dbRecipes, products)
                          : null;
                        const recipeKindLabel =
                          meal?.recipe_kind === "sweet"
                            ? "Sucré"
                            : meal?.recipe_kind === "savory"
                              ? "Salé"
                              : "Libre";
                                ...
                                <div className="planning-meal-actions">
                                  {meal.consumed_at ? (
                                    ...
                                    <button
                                      ...
                                      className="btn-tertiary"
                                      onClick={() =>
                                        void markWeekMealConsumed(meal)
                                      }
                                    >
                                      Valider
                                    </button>
                                  ...
                                  <button
                                    ...
                                    className="btn-tertiary"
                                    onClick={() =>
                                      void deleteWeekMeal(dateKey, slot.key)
                                    }
                                  >
                                    Supprimer
                                  </button>
          ...
          <div className="modal-backdrop">
            <div className="modal-card planner-modal-card">
              ...
              <p className="modal-subtitle">
                Choisis une recette, ajuste les portions, ou note un repas libre
              ...
              <div className="modal-context-row">
                ...
                <span>
                  {planRecipeValue ? "Recette sélectionnée" : "À compléter"}
                </span>
              </div>
                  ...
                  <>
                    ...
                    <div className="field-group full">
                      <label className="field-label">
                        Ingrédients du repas libre
                      </label>
                      <textarea
                        className="field-input"
                        value={planCustomIngredients}
                        onChange={(e) =>
                          setPlanCustomIngredients(e.target.value)
                        }
  ...
  const hideFromShopping = async (productId: string) => {
    ...
    setProducts((prev) =>
      prev.map((p) =>
        p.id === productId ? { ...p, shopping_hidden: true } : p,
      ),
    );
  ...
  const renderStockTab = () => {
    ...
    const getShelfKind = (placeName: string): StockShelfKind => {
      ...
      if (key.includes("frigo") || key.includes("refrigerateur"))
        return "fridge";
      if (key.includes("congel")) return "freezer";
      if (
        key.includes("menager") ||
        key.includes("entretien") ||
        key.includes("maison")
      )
        return "household";
      if (
    ...
    const extraStockPlaces = stockPlaces.filter(
      (placeName) =>
        !DEFAULT_STOCK_PLACES.some(
          (defaultPlace) => defaultPlace === placeName,
        ),
    );
    ...
    const stockShelfCards = stockShelfPlaces.map((placeName) => {
      ...
      const lowCount = items.filter(
        (item) => (item.quantity ?? 0) <= stepForUnit(item.unit),
      ).length;
    ...
    const filteredInStock = searchFilteredInStock.filter((item) => {
      ...
      if (stockPlaceFilter && item.place?.trim() !== stockPlaceFilter)
        return false;
      if (stockFilter === "soon") return status === "soon";
      if (stockFilter === "expired") return status === "expired";
      if (stockFilter === "expired-dlc")
        return status === "expired" && item.expiration_type === "dlc";
      if (stockFilter === "expired-ddm")
        return status === "expired" && item.expiration_type === "ddm";
      if (stockFilter === "open") return item.is_open;
      if (stockFilter === "low")
        return (item.quantity ?? 0) <= stepForUnit(item.unit);
    ...
    const sortedInStock = [...filteredInStock].sort((a, b) => {
      ...
      if (stockSort === "name") {
        ...
        result = (a.expiration_date ?? "9999-12-31").localeCompare(
          b.expiration_date ?? "9999-12-31",
        );
      }
    ...
    const uncategorizedItems = sortedInStock.filter(
      (item) => !categorizedStockIds.has(item.id),
    );
      ...
      return (
        <article key={item.id} className={`stock-product-card${cardTone}`}>
          <div className="stock-product-top">
            ...
            <span
              className={`status-pill ${getExpirationClass(status, item.expiration_type)}`}
            >
          ...
          <div className="stock-product-footer">
            {item.product ? (
              ...
              <span className="stock-product-toggle stock-product-toggle--muted">
                Produit lié manquant
              </span>
            )}
            ...
            <button
              ...
              onClick={() =>
                void updateStock(item.id, { is_open: !item.is_open })
              }
              title={
                item.is_open
    ...
    const renderStockCardGrid = (rows: StockItem[]) => (
      <div className="stock-product-grid">{rows.map(renderStockCard)}</div>
    );
      ...
      <>
        ...
        <section className="stock-hero">
          ...
          <div className="stock-hero-content">
            ...
            <div className="hero-actions">
              ...
              >
                <span
                  className="btn-symbol btn-symbol--scan"
                  aria-hidden="true"
                />
                Scanner un produit
              </button>
              ...
              >
                <span
                  className="btn-symbol btn-symbol--plus"
                  aria-hidden="true"
                />
                Ajouter au stock
              </button>
        ...
        <section className="card stock-command-card">
          <div className="stock-command-layout">
            ...
            <div className="field-group">
              ...
              <select
                ...
                onChange={(e) =>
                  setStockSortDirection(e.target.value as StockSortDirection)
                }
              >
                <option value="asc">
                  {stockSort === "expiration"
          ...
          <div className="subtabs stock-filter-tabs">
            {stockFilterOptions.map((option) => (
              <button
                ...
                className={
                  "subtab-btn" +
                  (stockFilter === option.value ? " subtab-btn--active" : "")
                }
          ...
          <div className="stock-command-footer">
            <p className="muted">
              {pluralize(sortedInStock.length, "résultat")} sur{" "}
              {pluralize(inStock.length, "produit")} en stock.
            </p>
        ...
        {loading ? (
          ...
          <section className="stock-empty-state">
            ...
            <div>
              ...
              <p>
                Commence par scanner un produit ou ajoute-le à la main. Ensuite
                VPlacard pourra remplir les rayons, repérer les dates et
  ...
  const renderDashboard = () => {
    ...
    const nextMeal = todayMeals.find(
      ({ meal }) => meal && !meal.consumed_at,
    )?.meal;
    ...
    return (
      <>
          ...
          <div className="today-hero-content">
            ...
            <div className="hero-actions">
              <button
                ...
                onClick={
                  heroItem
                    ? openRecipesFromDashboard
                    : openPlanTodayFromDashboard
                }
              >
                <span
                  className={`btn-symbol ${heroItem ? "btn-symbol--cook" : "btn-symbol--calendar"}`}
                  aria-hidden="true"
                />
            ...
            <div className="stat-progress">
              <div
                className={
                  remainingCalories >= 0
                    ? "stat-progress-fill"
                    : "stat-progress-fill stat-progress-fill-danger"
                }
        ...
        <section className="stats-row">
          ...
          <div className="stat-card">
            ...
            <div className="stat-foot">
              Sur les {settings.soonDays} prochains jours
            </div>
          </div>
          <div className="stat-card">
          ...
          <div className="stat-card">
            ...
            {plannedTodayCalories > 0 && (
              <div className="stat-foot">
                Prévu : +{plannedTodayCalories} kcal · total{" "}
                {projectedTodayCalories} kcal
              </div>
            )}
          </div>
        </section>
        ...
        <section className="card card-soft">
          ...
          <p className="section-subtitle">
            Les produits qui méritent de passer dans l'assiette.
          </p>
          ...
          <div className="field-row" style={{ marginBottom: "0.75rem" }}>
            <button
              type="button"
              className="btn-tertiary"
              onClick={() => openStockFromDashboard("soon")}
            >
            ...
            <button
              type="button"
              className="btn-tertiary"
              onClick={openRecipesFromDashboard}
            >
              Trouver une idée
            </button>
          </div>
          ...
          {priorityList.length === 0 ? (
            ...
            <ul className="priority-list">
              {priorityList.map((item) => {
                ...
                const labelStatus = getExpirationLabel(
                  status,
                  item.expiration_type,
                );
                const displayStatusLabel =
                  status === "ok" && item.is_open ? "Ouvert" : labelStatus;
                const displayStatusClass =
                  status === "ok" && item.is_open
                    ? "status-soon"
                    : getExpirationClass(status, item.expiration_type);
        ...
        <section className="dashboard-grid">
          ...
          <section className="card">
            ...
            <div className="field-row" style={{ marginBottom: "0.75rem" }}>
              <button
                type="button"
                className="btn-tertiary"
                onClick={() => openStockFromDashboard("expired-dlc")}
              >
          ...
          <section className="card card-soft">
            ...
            <p className="section-subtitle">
              À vérifier, souvent encore consommable.
            </p>
            ...
            <div className="field-row" style={{ marginBottom: "0.75rem" }}>
              <button
                type="button"
                className="btn-tertiary"
                onClick={() => openStockFromDashboard("expired-ddm")}
              >
        ...
        <section className="card">
          ...
          <div className="field-row" style={{ marginBottom: "0.75rem" }}>
            <button
              type="button"
              className="btn-tertiary"
              onClick={openTodayMealsFromDashboard}
            >
              Ouvrir le menu
            </button>
            <button
              type="button"
              className="btn-tertiary btn-with-icon"
              onClick={openPlanTodayFromDashboard}
            >
              <span
                className="btn-symbol btn-symbol--calendar"
                aria-hidden="true"
            ...
            <ul className="dashboard-meal-list">
              {todayMeals.map(({ slot, meal }) => (
                <li key={slot.key} className="dashboard-meal-item">
                  ...
                  <span className="dashboard-meal-name">
                    {meal?.recipe_name ?? "-"}
                  </span>
                  {meal && (
                    <span
                      className={
                        meal.consumed_at
                          ...
                          : "dashboard-meal-status dashboard-meal-status--planned"
                      }
                    >
                  ...
                  {meal &&
                    (() => {
                      const calories = mealCaloriesInfo(
                        meal,
                        dbRecipes,
                        products,
                      );
        ...
        <section className="card">
          ...
          <div className="field-row" style={{ marginBottom: "0.75rem" }}>
            <button
              type="button"
              className="btn-tertiary"
              onClick={() => openStockFromDashboard("low")}
            >
            ...
            <button
              type="button"
              className="btn-tertiary"
              onClick={openShoppingFromDashboard}
            >
        ...
        <section className="card add-product-panel" ref={addProductSectionRef}>
          <div className="form-panel-head">
            <div>
              ...
              <p className="section-subtitle">
                Ajout rapide depuis le tableau de bord avec scanner et
                auto-remplissage OpenFoodFacts.
              </p>
            </div>
            <button
              type="button"
          ...
          {addExistingProductId && (
            <div className="info-text" style={{ marginBottom: "0.75rem" }}>
              Produit existant sélectionné depuis la liste de courses. Tu
              ajoutes un nouveau lot à ce produit.
              <button
                type="button"
                className="btn-tertiary"
          ...
          <form className="form-grid product-form-grid" onSubmit={handleAdd}>
            ...
            <div className="field-group">
              ...
              <div className="date-shortcuts">
                <button
                  ...
                  className="btn-tertiary"
                  onClick={() =>
                    setExpiration(toDateKey(addMonths(new Date(), 1)))
                  }
                >
                  +1 mois
                </button>
                <button
                  ...
                  className="btn-tertiary"
                  onClick={() =>
                    setExpiration(toDateKey(addMonths(new Date(), 3)))
                  }
                >
                  +3 mois
                </button>
                <button
                  ...
                  className="btn-tertiary"
                  onClick={() =>
                    setExpiration(toDateKey(addMonths(new Date(), 6)))
                  }
                >
                  +6 mois
                </button>
                <button
                  ...
                  className="btn-tertiary"
                  onClick={() =>
                    setExpiration(toDateKey(endOfYear(new Date())))
                  }
                >
                  Fin d'année
                </button>
            ...
            <div className="field-group">
              ...
              <select
                ...
                onChange={(e) =>
                  setExpirationType(e.target.value as ExpirationType)
                }
                disabled={!expiration}
              >
  ...
  const renderShoppingTab = () => {
    ...
    const plannedMissingIngredientsByName = new Map<
      string,
      RecipeIngredient[]
    >();

    const stockQuantityForShoppingIngredient = (
      stock: StockItem,
      ingredient: RecipeIngredient,
    ): number | null => {
    ...
    const matchingStocksForShoppingIngredient = (
      ingredient: RecipeIngredient,
    ) => {
    ...
    const getMissingShoppingIngredient = (
      ingredient: RecipeIngredient,
      ...
      const missingAmount = roundQty(
        ingredient.amount - available,
        ingredient.unit,
      );
    ...
    const addPlannedMissingIngredient = (ingredient: RecipeIngredient) => {
      ...
      if (ingredient.amount != null && ingredient.unit) {
        ...
        if (sameUnitIndex >= 0) {
          ...
          copy[sameUnitIndex] = {
            ...existing,
            amount: roundQty(
              (existing.amount ?? 0) + ingredient.amount,
              ingredient.unit,
            ),
          };
      ...
      if (current.some((item) => formatRecipeIngredient(item) === label))
        return;
    ...
    const lowStockByProductId = new Map(
      lowStockList.filter((s) => s.product?.id).map((s) => [s.product!.id, s]),
    );

    const plannedMissingProductIds = new Set(
      products
        .filter((product) =>
          plannedMissingNames.has(normalizeText(getProductMatchName(product))),
        )
    ...
    const missingMain = products.filter(
      (p) => p.is_main && !p.shopping_hidden && !presentProductIds.has(p.id),
    );
    ...
    const getShoppingReasonClass = (product: Product) => {
      ...
      if (reason === "Stock faible + repas")
        return "shopping-product-reason--combo";
      if (reason === "Stock faible") return "shopping-product-reason--low";
      if (reason === "Repas planifié")
        return "shopping-product-reason--planned";
    ...
    const displayedShoppingList = hideCheckedShopping
      ? visibleShoppingList.filter(
          (product) => !checkedShoppingIds.includes(product.id),
        )
      : visibleShoppingList;
    ...
    const startAddFromShopping = (product: Product) => {
      const categoryValue = MAIN_CATEGORIES.includes(
      ...
      setQuantity(
        plannedIngredient?.amount != null
          ? String(plannedIngredient.amount)
          : "1",
      );
      ...
      setGramsPerUnit(
        product.grams_per_unit_g != null
          ? String(product.grams_per_unit_g)
          : "",
      );
      setDensityGml(
        product.density_g_ml != null ? String(product.density_g_ml) : "",
      );
      setError(null);
      setActiveTab("dashboard");
      setScrollToAddForm(true);
      setInfo(
        "Formulaire prérempli depuis la liste de courses. Complète la quantité, le lieu et la date.",
      );
    };
    ...
    const remainingShoppingCount = Math.max(
      0,
      visibleShoppingList.length - visibleCheckedCount,
    );
    ...
    const title =
      shoppingSubTab === "main"
        ? "Essentiels à racheter"
        : "Compléments à racheter";
    const subtitle =
      shoppingSubTab === "main"
      ...
      <>
        <div className="main-header">
          <div>
            ...
            <p className="main-subtitle">
              La liste utile, construite avec ton stock et ton planning.
            </p>
          </div>
          <div className="main-header-right">
        ...
        <section className="shopping-trip-band">
          ...
          <div className="shopping-trip-content">
            ...
            <p>
              {pluralize(plannedMissingCount, "produit")} pour les repas prévus
              ·{" "}
              {pluralize(
                lowStockShoppingCount,
                "stock faible",
                "stocks faibles",
              )}{" "}
              · {pluralize(visibleCheckedCount, "déjà coché")}
            </p>
        ...
        <section className="shopping-command-panel">
          ...
          {visibleShoppingList.length > 0 && (
            <p className="muted shopping-command-count">
              {pluralize(visibleCheckedCount, "produit")} coché
              {visibleCheckedCount > 1 ? "s" : ""} sur{" "}
              {pluralize(visibleShoppingList.length, "article")} visible
              {visibleShoppingList.length > 1 ? "s" : ""}.
            </p>
          )}
        </section>
                              ...
                              >
                                ...
                                <div className="shopping-item-main">
                                  <div className="shopping-item-heading">
                                    ...
                                    <span
                                      className={`shopping-product-reason ${getShoppingReasonClass(p)}`}
                                    >
                                      {reason}
                                    </span>
                                  </div>
                                  ...
                                  <div className="shopping-item-details">
                                    ...
                                    {plannedLabel && (
                                      <span>À prévoir : {plannedLabel}</span>
                                    )}
                                    {p.sub_category && (
  ...
  const renderSettingsTab = () => {
          ...
          <div className="field-group">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setSettings((prev) => ({
                  ...prev,
                  dailyCalorieGoal: Number.isFinite(v)
                    ? v
                    : prev.dailyCalorieGoal,
                }));
                setSettingsInfo(null);
    ...
    const customizableRecipes = [...dbRecipes, ...SAMPLE_RECIPES].map(
    ...
    const changeSettingsImage = async (
      ...
      try {
        ...
        setError(
          getErrorMessage(uploadError) ?? "Impossible d'envoyer cette photo.",
        );
      } finally {
    ...
    const resetSettingsImage = async (
      kind: ImageAssetKind,
      imageKey: string,
    ) => {
      const uploadKey = `${kind}:${imageKey}`;
      ...
      try {
        ...
        setError(
          getErrorMessage(resetError) ??
            "Impossible de réinitialiser cette photo.",
        );
      } finally {
        setImageUploadingKey(null);
      }
      ...
      <>
        <div className="main-header">
          <div>
            ...
            <p className="main-subtitle">
              Tes préférences de stock, recettes et suivi du jour.
            </p>
          </div>
          <div className="main-header-right">
        ...
        <section className="settings-hero">
          <div>
            ...
            <p>
              Ces valeurs pilotent les alertes, les recettes proposées, les
              calories et les lieux par défaut.
            </p>
          </div>
          <div className="settings-hero-stats">
            <span>
              <strong>{settings.soonDays}</strong>jours
            </span>
            <span>
              <strong>{settings.recipesMaxMissing}</strong>manquants
            </span>
            <span>
              <strong>{settings.dailyCalorieGoal}</strong>kcal
            </span>
          </div>
        </section>
        ...
        <section className="settings-image-panel">
          <div className="settings-section-head">
            <div>
              ...
              <p>
                Choisis des fichiers image : ils seront envoyés dans le bucket
                Supabase de l'app.
              </p>
            </div>
          </div>
            ...
            {APP_IMAGE_FIELDS.map((field) => (
              <article key={field.key} className="image-setting-card">
                <div
                  ...
                  style={
                    {
                      "--image-setting-preview": cssImageUrl(
                        appImages[field.key],
                      ),
                    } as CSSProperties
                  }
                ...
                <div className="image-setting-body">
                  ...
                  <div className="field-group">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        void changeSettingsImage(event, "app", field.key)
                      }
                    />
                  </div>
                  <button
                  ...
                  >
                    {imageUploadingKey === `app:${field.key}`
                      ? "Envoi..."
                      : "Réinitialiser"}
                  </button>
                </div>
        ...
        <section className="settings-image-panel">
          <div className="settings-section-head">
            <div>
              ...
              <p>
                Chaque recette peut avoir sa propre photo hébergée dans
                Supabase.
              </p>
            </div>
          </div>
              ...
              return (
                <article key={recipe.id} className="recipe-image-setting-card">
                  <div
                    ...
                    style={
                      {
                        "--image-setting-preview": cssImageUrl(currentImage),
                      } as CSSProperties
                    }
                    aria-hidden="true"
                  />
                  <div className="recipe-image-setting-body">
                    <div>
                      <h3>{recipe.name}</h3>
                      <p>
                        {recipe.kind === "savory" ? "Salé" : "Sucré"} ·{" "}
                        {pluralize(recipe.ingredients.length, "ingrédient")}
                      </p>
                    </div>
                    <div className="field-group">
                      <label className="field-label">Remplacer la photo</label>
                      <div className="field-row">
                        <input
                          ...
                          onChange={(event) =>
                            void changeSettingsImage(event, "recipe", recipe.id)
                          }
                        />
                        {recipeImages[recipe.id] && (
                          <button
                            ...
                            className="btn-tertiary"
                            disabled={
                              imageUploadingKey === `recipe:${recipe.id}`
                            }
                            onClick={() =>
                              void resetSettingsImage("recipe", recipe.id)
                            }
                          >
                            {imageUploadingKey === `recipe:${recipe.id}`
                              ? "Envoi..."
        ...
        <section className="settings-save-panel">
          <div>
            ...
            <p>
              Les réglages rapides restent sur cet appareil. Les photos et
              recettes personnalisées sont synchronisées avec Supabase.
            </p>
          </div>
          <button
            type="button"
  ...
  const renderRecipesTab = () => {
    ...
    const stockQuantityInIngredientUnit = (
      stock: StockItem,
      ingredient: RecipeIngredient,
    ): number | null => {
      const qty = stock.quantity ?? 0;
    ...
    const allRecipes: Recipe[] = [...dbRecipes, ...SAMPLE_RECIPES].map(
      applyRecipeOverrides,
    );
      ...
      const urgentIngredients = r.ingredients
        .filter(
          (ing: RecipeIngredient) =>
            hasEnoughIngredient(ing) && usesUrgentIngredient(ing),
        )
        .map((ing: RecipeIngredient) => ing.name);
    ...
    const feasibleList = filteredRecipes
      ...
      .sort(
        (a, b) =>
          b.urgentCount - a.urgentCount || a.missingCount - b.missingCount,
      );
    ...
    const readyRecipesCount = enriched.filter(
      (recipe) => recipe.missingCount === 0,
    ).length;
    const urgentRecipesCount = enriched.filter(
      (recipe) => recipe.urgentCount > 0,
    ).length;
    ...
    const headerTitle = showRecipeInspiration
      ? "Inspirations pour démarrer"
      : recipesSubTab === "feasible"
        ? `Faisable avec ton stock`
        : "Toutes tes idées";

    const headerSubtitle = showRecipeInspiration
    ...
    const renderRecipeCard = (r: EnrichedRecipe) => {
      ...
      const remainingIngredients =
        r.ingredients.length - visibleIngredients.length;
      const tagLabel =
        r.tags?.[0] ?? (r.kind === "savory" ? "Plat" : "Dessert");
        ...
        >
          <div className="recipe-card-media">
            <div className="recipe-media-top">
              <span className="recipe-kind-pill">
                {r.kind === "savory" ? "Salé" : "Sucré"}
              </span>
              <span className={"recipe-readiness-pill" + readinessClass}>
                {readinessLabel}
              </span>
            </div>
            <div className="recipe-media-bottom">
              <span>{tagLabel}</span>
              <span>
                {r.servings ? pluralize(r.servings, "portion") : "Recette"}
              </span>
            </div>
          </div>
          ...
          <div className="recipe-card-body">
            <div className="recipe-head">
              ...
              <div className="recipe-head-actions">
                ...
                >
                  <span
                    className="btn-symbol btn-symbol--edit"
                    aria-hidden="true"
                  />
                  Modifier
                </button>
                {isDbRecipe && (
                  ...
                  >
                    <span
                      className="btn-symbol btn-symbol--trash"
                      aria-hidden="true"
                    />
                    Supprimer
                  </button>
            ...
            {recipesSubTab === "feasible" && (
              <div className="recipe-stock-line">
                <span className="recipe-badge">
                  {isReady ? "Tout est disponible" : badge}
                </span>
              </div>
            )}
            ...
            {remainingIngredients > 0 && (
              <p className="recipe-more">
                + {pluralize(remainingIngredients, "ingrédient")}
              </p>
            )}

            {kcalR.missingCount > 0 && (
              <p className="recipe-meta">
                {pluralize(kcalR.missingCount, "ingrédient")} sans calcul
                nutritionnel.
              </p>
            )}
      ...
      <>
        ...
        <section className="cook-band">
          <div>
            ...
            <p>
              L'app repère ce que tu as, ce qui expire et ce qu'il manque pour
              transformer ton stock en repas.
            </p>
          </div>
          <div className="cook-band-side">
            ...
            >
              <span
                className="btn-symbol btn-symbol--cook"
                aria-hidden="true"
              />
              Créer une recette
            </button>
        ...
        <section className="card">
          <div className="field-group full">
            ...
            <div className="subtabs" style={{ marginTop: "0.6rem" }}>
              {recipeFilterOptions.map((option) => (
                <button
                  ...
                  className={
                    "subtab-btn" +
                    (recipeFilter === option.value ? " subtab-btn--active" : "")
                  }
            ...
            <p className="muted" style={{ marginTop: "0.4rem" }}>
              {pluralize(current.length, "recette")} affichée
              {current.length > 1 ? "s" : ""} sur {enriched.length}.
            </p>
          </div>
        </section>
  ...
  const renderPlaceholder = (title: string, description: string) => (
    <>
      ...
      <section className="card">
        <p className="muted">
          Cette section n'est pas disponible depuis la navigation actuelle.
        </p>
      </section>
    </>
  );
      ...
      {newRecipeOpen && (
        <div className="modal-backdrop">
          <div className="modal-card recipe-modal-card">
            ...
            <p className="modal-subtitle">
              Ajoute une idée simple, puis VPlacard la comparera au stock pour
              les menus et les courses.
            </p>
              ...
              style={
                {
                  "--modal-image": cssImageUrl(
                    newRecipeImagePreview ||
                      RECIPE_IMAGE_POOL[newRecipeKind][0],
                  ),
                } as CSSProperties
              }
              aria-hidden="true"
            ...
            <div className="form-grid" style={{ marginTop: "0.85rem" }}>
              ...
              <div className="field-group">
                ...
                <select
                  ...
                  value={newRecipeKind}
                  onChange={(e) =>
                    setNewRecipeKind(e.target.value as RecipeKind)
                  }
                >
                ...
                <div className="field-row">
                  ...
                  {newRecipeImageFile && (
                    <button
                      type="button"
                      className="btn-tertiary"
                      onClick={() => setNewRecipeImageSelection(null)}
                    >
                      Retirer
                    </button>
                  )}
      ...
      {editRecipeOpen && editRecipeTarget && (
        <div className="modal-backdrop">
          <div className="modal-card recipe-modal-card">
            ...
            <p className="modal-subtitle">
              Ajuste les portions et les ingrédients pour que les suggestions de
              repas restent fiables.
            </p>
              ...
              style={
                {
                  "--modal-image": cssImageUrl(
                    editRecipeImagePreview ||
                      (editRecipeTarget
                        ? recipeImageFor(editRecipeTarget, recipeImages)
                        : RECIPE_IMAGE_POOL[editRecipeKind][0]),
                  ),
                } as CSSProperties
              }
              aria-hidden="true"
            ...
            <div className="form-grid modal-form-grid">
              ...
              <div className="field-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditRecipeKind(e.target.value as RecipeKind)
                  }
                >
                  <option value="savory">Salé</option>
                  <option value="sweet">Sucré</option>
                ...
                <div className="field-row">
                  ...
                  {editRecipeImageFile && (
                    <button
                      type="button"
                      className="btn-tertiary"
                      onClick={() => setEditRecipeImageSelection(null)}
                    >
                      Retirer
                    </button>
                  )}
      ...
      {editOpen && editTarget && (
        <div className="modal-backdrop">
          <div className="modal-card product-modal-card">
            ...
            <p className="modal-subtitle">
              Ajuste l'identité, l'emplacement, les quantités et les infos
              nutritionnelles utilisées par les recettes.
            </p>
            ...
            <div className="form-grid modal-form-grid product-modal-grid">
              ...
              <div className="field-group">
                ...
                <select
                  ...
                  value={editCategory}
                  onChange={(e) =>
                    setEditCategory(e.target.value as MainCategory | "")
                  }
                >
                  <option value="">(Aucune)</option>
              ...
              <div className="field-group">
                ...
                <button
                  type="button"
                  className="btn-tertiary field-inline-action"
              ...
              <div className="field-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditExpirationType(e.target.value as ExpirationType)
                  }
        ...
        <aside className="sidebar">
          ...
          <nav className="sidebar-nav">
            {navItems.map((item) => (
              ...
              >
                <span
                  className={`sidebar-item-icon sidebar-item-icon--${item.icon}`}
                  aria-hidden="true"
                />
          ...
          <div className="sidebar-footer">
            <div className="sidebar-footer-box">
              ...
              <button
                type="button"
                className="sidebar-scan-btn btn-with-icon"
                onClick={() => setShowScanner(true)}
              >
                <span
                  className="btn-symbol btn-symbol--scan"
